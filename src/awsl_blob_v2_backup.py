...
@router.get("/v2_backup/list", response_model=List[BlobItem], responses={404: {"model": Message}}, tags=["AwslV2Backup"])
def awsl_list(uid: Optional[str] = "", limit: Optional[int] = 10, offset: Optional[int] = 0):
    ...
    return DBClientBase.get_client().awsl_list(uid, limit, offset)


@router.get("/v2_backup/list_count", response_model=int, tags=["AwslV2Backup"])
def awsl_list_count(uid: Optional[str] = "") -> int:
    return DBClientBase.get_client().awsl_list_count(uid)


@router.get("/v2_backup/random", response_model=str, tags=["AwslV2Backup"])
def awsl_random(uid: Optional[str] = "") -> str:
    return DBClientBase.get_client().awsl_random(uid)


@router.get("/v2_backup/random_json", response_model=BlobItem, tags=["AwslV2Backup"])
